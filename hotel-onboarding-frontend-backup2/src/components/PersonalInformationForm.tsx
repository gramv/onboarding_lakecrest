...
import { SSNInput, PhoneInput } from '@/components/ui/MaskedInput'
import { FederalComplianceValidator } from '@/services/FederalComplianceValidator'
import { secureStorage } from '@/services/SecureStorageService'
import { SSNValidationAlert } from '@/components/ui/ComplianceAlert'
...
export default function PersonalInformationForm({
  ...
  const validateForm = () => {
    ...
    // Enhanced SSN validation using FederalComplianceValidator
    if (formData.ssn) {
      const ssnValidation = FederalComplianceValidator.validateSSN(formData.ssn)
      if (!ssnValidation.isValid) {
        newErrors.ssn = ssnValidation.errors[0] || 'Invalid SSN format'
      }
      if (ssnValidation.warnings.length > 0) {
        newWarnings.ssn = ssnValidation.warnings[0]
      }
    }
          ...
          <div className="grid-responsive-forms">
            ...
            <div className="input-group">
              <SSNInput
                ...
                label={`${t('ssn')}`}
                required={true}
                error={shouldShowError('ssn') ? errors.ssn : undefined}
                helperText={language === 'en' ? 'Your SSN is encrypted and secure' : 'Su SSN está encriptado y seguro'}
                showToggle={true}
                autoMaskOnBlur={true}
                containerClassName="w-full"
                inputClassName="form-input-enhanced"
              />
            </div>
            <div className="input-group">
              <PhoneInput
