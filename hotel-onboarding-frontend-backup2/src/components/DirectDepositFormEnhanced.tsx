...
import { AccountNumberInput, RoutingNumberInput } from '@/components/ui/MaskedInput'
import { FederalComplianceValidator } from '@/services/FederalComplianceValidator'
...
export default function DirectDepositFormEnhanced({
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (formData.paymentMethod === 'direct_deposit') {
      ...
      } else if (!FederalComplianceValidator.validateRoutingNumber(formData.primaryAccount.routingNumber)) {
              ...
              <div>
                <div className="relative">
                  <RoutingNumberInput
                    ...
                    onChange={(value) => handleInputChange('primaryAccount.routingNumber', value)}
                    onBlur={() => handleFieldBlur('primaryAccount.routingNumber')}
                    label={`${t('routing_number')}`}
                    required={true}
                    error={shouldShowError('primaryAccount.routingNumber') ? errors['primaryAccount.routingNumber'] : undefined}
                    helperText={language === 'en' ? '9-digit routing number' : 'Número de ruta de 9 dígitos'}
                    showToggle={true}
                    autoMaskOnBlur={true}
                    containerClassName="w-full"
                  />
                  {primaryRoutingValidation.isValidating && (
                    <Loader2 className="absolute right-12 top-9 h-4 w-4 text-gray-400 animate-spin" />
                  )}
                  {!primaryRoutingValidation.isValidating && 
                   primaryRoutingValidation.validationResult?.valid && 
                   formData.primaryAccount.routingNumber.length === 9 && (
                    <Check className="absolute right-12 top-9 h-4 w-4 text-green-600" />
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <AccountNumberInput
                    ...
                    onChange={(value) => handleInputChange('primaryAccount.accountNumber', value)}
                    onBlur={() => handleFieldBlur('primaryAccount.accountNumber')}
                    label={`${t('account_number')}`}
                    required={true}
                    error={shouldShowError('primaryAccount.accountNumber') ? errors['primaryAccount.accountNumber'] : undefined}
                    helperText={language === 'en' ? 'Your account is secure' : 'Su cuenta está segura'}
                    showToggle={true}
                    autoMaskOnBlur={true}
                    containerClassName="w-full"
                  />
                ...
                <div>
                  <AccountNumberInput
                    ...
                    onChange={(value) => handleInputChange('primaryAccount.accountNumberConfirm', value)}
                    onBlur={() => handleFieldBlur('primaryAccount.accountNumberConfirm')}
                    label={`${t('confirm_account')}`}
                    required={true}
                    error={shouldShowError('primaryAccount.accountNumberConfirm') ? errors['primaryAccount.accountNumberConfirm'] : undefined}
                    showToggle={true}
                    autoMaskOnBlur={false}
                    containerClassName="w-full"
                  />
                </div>
              </div>
