...
import { CheckCircle, FileText, Upload, Camera, Globe, AlertTriangle, Clock } from 'lucide-react'
...
import { ComplianceAlert, ComplianceAlertGroup } from '@/components/ui/ComplianceAlert'
...
import { formatDeadline } from '@/utils/businessDayCalculator'
...
export default function I9CompleteStep({
  ...
  const complianceValidation = useComplianceValidation({ language, debounceMs: 300 })
  ...
  useEffect(() => {
    ...
    // Validate SSN format when it changes
    if (formData.ssn) {
      complianceValidation.validateSSN(formData.ssn)
    }
    
    // Validate age if we have date of birth
    if (formData.date_of_birth) {
      complianceValidation.validateAge(formData.date_of_birth, false)
    }
    
    // Check I-9 timing if we have hire date
    if (employee?.hireDate || employee?.hire_date || employee?.startDate) {
      const hireDate = employee.hireDate || employee.hire_date || employee.startDate
      complianceValidation.validateI9Timing(
        hireDate,
        isSigned ? new Date() : null,
        null
      )
    }
  }, [formData.citizenship_status, formData.ssn, formData.date_of_birth, documentsData, employee, isSigned])
  ...
  return (
    <StepContainer errors={errors} fieldErrors={fieldErrors} saveStatus={saveStatus}>
      <StepContentWrapper>
        ...
        {/* Compliance Warnings */}
        {complianceValidation.i9Deadlines && !isSigned && (
          <ComplianceAlert
            severity={
              complianceValidation.i9Deadlines.isSection1Overdue ? 'error' :
              complianceValidation.i9Deadlines.section1BusinessDaysRemaining <= 2 ? 'warning' :
              'info'
            }
            title={language === 'es' ? 'Fecha Límite del I-9' : 'I-9 Deadline'}
            messages={[
              language === 'es' 
                ? `Sección 1 debe completarse antes del: ${formatDeadline(complianceValidation.i9Deadlines.section1Deadline, complianceValidation.i9Deadlines.section1BusinessDaysRemaining, language)}`
                : `Section 1 must be completed by: ${formatDeadline(complianceValidation.i9Deadlines.section1Deadline, complianceValidation.i9Deadlines.section1BusinessDaysRemaining, language)}`,
              language === 'es'
                ? `Sección 2 debe completarse antes del: ${formatDeadline(complianceValidation.i9Deadlines.section2Deadline, complianceValidation.i9Deadlines.section2BusinessDaysRemaining, language)}`
                : `Section 2 must be completed by: ${formatDeadline(complianceValidation.i9Deadlines.section2Deadline, complianceValidation.i9Deadlines.section2BusinessDaysRemaining, language)}`
            ]}
            regulationLink={{
              text: language === 'es' ? 'Ver Requisitos del USCIS' : 'View USCIS Requirements',
        ...
        {complianceValidation.ssnValidation && !complianceValidation.ssnValidation.isValid && (
          <ComplianceAlert
            severity="error"
            title={language === 'es' ? 'Problema con el SSN' : 'SSN Issue'}
            messages={complianceValidation.ssnValidation.errors}
            dismissible={false}
          />
        )}
        
        {/* Age Validation Warnings */}
        {complianceValidation.ageValidation && !complianceValidation.ageValidation.isValid && (
          <ComplianceAlert
            severity="error"
            title={language === 'es' ? 'Requisito de Edad' : 'Age Requirement'}
            messages={complianceValidation.ageValidation.errors}
            dismissible={false}
          />
        )}
