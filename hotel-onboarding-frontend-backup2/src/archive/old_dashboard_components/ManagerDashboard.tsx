...
export default function ManagerDashboard() {
  ...
  const fetchPropertyData = async () => {
    ...
    const response = await axios.get('/manager/property', {
  ...
  const fetchDashboardStats = async () => {
    ...
    const token = localStorage.getItem('token')
    const appsResponse = await axios.get('/manager/applications', {
      headers: { Authorization: `Bearer ${token}` }
    })
    ...
    <ErrorBoundary>
      <div className="responsive-container padding-md">
        ...
        <TabsContent value="applications">
          <ApplicationsTab userRole="manager" propertyId={property?.id} />
        </TabsContent>
